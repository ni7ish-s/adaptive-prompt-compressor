...
The pipeline currently runs four compression stages in sequence:
...
All compression thresholds were tuned against a fixed quality floor (BERTScore F1 ≥ 0.88) rather than optimized for raw token savings alone — a configuration that saves more tokens but drops below that floor is treated as a regression, not an improvement.
...
- Compression thresholds are currently global rather than content-type-aware — the same settings are applied to chat logs and prose alike, even though they behave differently under compression. The content-type router (see roadmap) is the planned fix.
...
- `sentence-transformers` — local embedding model for semantic deduplication
